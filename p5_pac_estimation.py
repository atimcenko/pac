...
def main():
    ...
    pickle_filepath = os.path.join(p5_root_dir, "Patient5.pkl")
    p5 = load_patient_from_pickle(pickle_filepath)

    print(f"Loaded {p5.name}")

    print("Conditions: \n") 
    print(*[s for s in p5.conditions if '180sec' in s], sep='\n')
    print("Placements: \n") 
    print(*p5.placements, sep='\n')
    ...
    for placement_phase in placements:
        ...
        for placement_amplitude in placements:
            for condition in conditions:
                ...
                lfp_phase = p5.lfp[condition][placement_phase]
                lfp_amplitude = p5.lfp[condition][placement_amplitude]
                
                # checking if file already exists
                pac_filename = create_pac_name(lfp_phase, lfp_amplitude) + ".pkl"
                print(pac_filename)
                if os.path.isfile(os.path.join(p5.root_dir, "pac", pac_filename)):
