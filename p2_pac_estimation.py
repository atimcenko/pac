...
def main():
    ...
    p2_pickle_filepath = r"C:\Users\aleks\[[Python]]\LAB\Patient2\Patient2.pkl"
    
    p2 = load_patient_from_pickle(p2_pickle_filepath)

    print(f"Loaded {p2.name}")

    print("Conditions: \n") 
    print(*[s for s in p2.conditions if '180sec' in s], sep='\n')
    print("Placements: \n") 
    print(*p2.placements, sep='\n')
    ...
    for condition in conditions:
        ...
        for placement_phase in placements:
            for placement_amplitude in placements:
                ...
                lfp_phase = p2.lfp[condition][placement_phase]
                lfp_amplitude = p2.lfp[condition][placement_amplitude]
                
                # checking if file already exists
                pac_filename = create_pac_name(lfp_phase, lfp_amplitude) + ".pkl"
                print(pac_filename)
                if os.path.isfile(os.path.join(p2.root_dir, "pac", pac_filename)):
